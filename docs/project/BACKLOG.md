...
| E10 | **`TestRetrievedMemoryOrdering.test_memory_block_preserves_score_order` is nondeterministic** | `tests/test_conversation_quality.py`'s `_result()` helper stamps each fixture's `MemoryTurn.created_at` with its own `datetime.now(UTC)` call, three calls in a row. The formatter it exercises (`ContextBuilder._format_memory_block`, `context_builder.py:614-616`) deliberately sorts by `created_at` descending ("most recent first" is in the prompt text) — it does not preserve score order at all, despite the test's docstring. The test only passes because the three `now(UTC)` calls usually tie, letting Python's stable sort fall back to input order by luck. Measured on the dev machine: ties ~99.93% of the time under low contention, but the miss rate rises under full-suite CPU contention, which is when it was observed to fail (`assert system.index("BEST") < system.index("MIDDLE")` failed with `MIDDLE` at index 2863 preceding `BEST` at 2918 — exactly the ordering predicted for a non-tied clock). Fix by giving `_result()` an explicit, strictly-increasing `created_at` per call (e.g. an offset parameter) so the test exercises a real, stable ordering instead of relying on timestamp collisions — and decide whether the test should assert *recency* order (matching the formatter) rather than *score* order, since as written the docstring's premise is incorrect regardless of the flake. Found during Batch 4A's read-only verification pass (2026-08-03); confirmed unrelated to that batch — no shared files, no shared state, no test-ordering plugin installed. |
